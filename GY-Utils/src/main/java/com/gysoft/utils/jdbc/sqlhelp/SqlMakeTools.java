...
import com.gysoft.utils.jdbc.bean.*;
...
import static com.gysoft.utils.jdbc.dao.EntityDao.*;
import static com.gysoft.utils.jdbc.sqlhelp.EntityTools.getColumnName;
...
public class SqlMakeTools {
    ...
    public static <E> String makeSql(Class clazz, String tbName, String sqlFlag) {
        ...
        if (sqlFlag.equals(SQL_INSERT)) {
            ...
            for (int i = 0; fields != null && i < fields.length; i++) {
                ...
                String column = getColumnName(fields[i]);//获取属性对应字段名，没有注解默认按照属性名。有Column注解，获取Column的name作为字段名
    ...
    public static void buildCriteriaTree(Criteria criteria, CriteriaTree criteriaTree) {
        ...
        for (int i = 0; i < criterias.size(); i++) {
            ...
            CriteriaTree cTree = CriteriaTree.builder().id(IdGenerator.newShortId()).sql(pair.getFirst()).params(pair.getSecond()).childCriteriaTree(new ArrayList<>()).build();
